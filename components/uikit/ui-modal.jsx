...
export const UiModal = ({
  ...
  const handleClick = (e) => {
    const isModal = e.target.closest("[data-id=ui-modal]");
    if (isModal) {
      return;
    }
    onClose();
    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="
            absolute w-8 h-8 flex justify-center 
            items-center bg-white/10 
            hover:bg-white/40 transition
            left-[calc(100%+12px)] rounded"
        >
...
UiModal.Header = function UiModalHeader({ children, className }) {
  return (
    <div className={clsx("px-6 pt-6 pb-4 text-2xl font-semibold", className)}>
      {children}
    </div>
  );
};
